...
internal class ClassesCoverageTest : KontureScopeTestFixture() {
    ...
    fun `test ClassesShouldPackageAssertions package, module, and name`() {
        ...
        ClassesRuleBuilder(
            graph,
        ).should().inPackage("wrong.pkg").getShouldAssertion()!!(classA, listOf(classA), vPkgSingle)
        assertEquals(1, vPkgSingle.size)

        val vPkgList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().inPackage(listOf("wrong.pkg")).getShouldAssertion()!!(classA, listOf(classA), vPkgList)
        assertEquals(1, vPkgList.size)

        val vPkgVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().inPackage("wrong.pkg", "other").getShouldAssertion()!!(classA, listOf(classA), vPkgVararg)
        assertEquals(1, vPkgVararg.size)

        val vNotPkgSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notInPackage("com.example").getShouldAssertion()!!(classA, listOf(classA), vNotPkgSingle)
        ...
        ClassesRuleBuilder(
            graph,
        ).should().inModule(":other").getShouldAssertion()!!(classA, listOf(classA), vModSingle)
        assertEquals(1, vModSingle.size)

        val vModList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().inModules(listOf(":other")).getShouldAssertion()!!(classA, listOf(classA), vModList)
        assertEquals(1, vModList.size)

        val vModVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().inModules(":other", ":wrong").getShouldAssertion()!!(classA, listOf(classA), vModVararg)
        assertEquals(1, vModVararg.size)

        val vNotModSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notInModule(":app").getShouldAssertion()!!(classA, listOf(classA), vNotModSingle)
        assertEquals(1, vNotModSingle.size)

        val vNotModList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notInModules(listOf(":app")).getShouldAssertion()!!(classA, listOf(classA), vNotModList)
        assertEquals(1, vNotModList.size)

        val vNotModVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notInModules(":app", ":other").getShouldAssertion()!!(classA, listOf(classA), vNotModVararg)
        assertEquals(1, vNotModVararg.size)

        val vNameSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().named("Other").getShouldAssertion()!!(classA, listOf(classA), vNameSingle)
        assertEquals(1, vNameSingle.size)

        val vNameList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().named(listOf("Other")).getShouldAssertion()!!(classA, listOf(classA), vNameList)
        assertEquals(1, vNameList.size)

        val vNameVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().named("Other", "Wrong").getShouldAssertion()!!(classA, listOf(classA), vNameVararg)
        assertEquals(1, vNameVararg.size)

        val vNotNameSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNamed("ClassA").getShouldAssertion()!!(classA, listOf(classA), vNotNameSingle)
        assertEquals(1, vNotNameSingle.size)

        val vNotNameList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNamed(listOf("ClassA")).getShouldAssertion()!!(classA, listOf(classA), vNotNameList)
        assertEquals(1, vNotNameList.size)

        val vNotNameVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNamed("ClassA", "Other").getShouldAssertion()!!(classA, listOf(classA), vNotNameVararg)
        assertEquals(1, vNotNameVararg.size)

        val vStartSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().nameStartsWith("Other").getShouldAssertion()!!(classA, listOf(classA), vStartSingle)
        assertEquals(1, vStartSingle.size)

        val vStartList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().nameStartsWith(listOf("Other")).getShouldAssertion()!!(classA, listOf(classA), vStartList)
        assertEquals(1, vStartList.size)

        val vStartVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().nameStartsWith("Other", "X").getShouldAssertion()!!(classA, listOf(classA), vStartVararg)
        assertEquals(1, vStartVararg.size)

        val vNotStartSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNameStartsWith("Class").getShouldAssertion()!!(classA, listOf(classA), vNotStartSingle)
        assertEquals(1, vNotStartSingle.size)

        val vNotStartList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNameStartsWith(listOf("Class")).getShouldAssertion()!!(
            classA,
            listOf(classA),
            vNotStartList,
        )
        ...
        ClassesRuleBuilder(
            graph,
        ).should().notNameStartsWith("Class", "Other").getShouldAssertion()!!(
            classA,
            listOf(classA),
            vNotStartVararg,
        )
        ...
        ClassesRuleBuilder(
            graph,
        ).should().nameEndsWith("Other").getShouldAssertion()!!(classA, listOf(classA), vEndSingle)
        assertEquals(1, vEndSingle.size)

        val vEndList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().nameEndsWith(listOf("Other")).getShouldAssertion()!!(classA, listOf(classA), vEndList)
        assertEquals(1, vEndList.size)

        val vEndVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().nameEndsWith("Other", "X").getShouldAssertion()!!(classA, listOf(classA), vEndVararg)
        assertEquals(1, vEndVararg.size)

        val vNotEndSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNameEndsWith("A").getShouldAssertion()!!(classA, listOf(classA), vNotEndSingle)
        assertEquals(1, vNotEndSingle.size)

        val vNotEndList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNameEndsWith(listOf("A")).getShouldAssertion()!!(classA, listOf(classA), vNotEndList)
        assertEquals(1, vNotEndList.size)

        val vNotEndVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNameEndsWith("A", "Other").getShouldAssertion()!!(classA, listOf(classA), vNotEndVararg)
        assertEquals(1, vNotEndVararg.size)

        val vMatchSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().nameMatches("Other*").getShouldAssertion()!!(classA, listOf(classA), vMatchSingle)
        assertEquals(1, vMatchSingle.size)

        val vMatchList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().nameMatches(listOf("Other*")).getShouldAssertion()!!(classA, listOf(classA), vMatchList)
        assertEquals(1, vMatchList.size)

        val vMatchVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().nameMatches("Other*", "X*").getShouldAssertion()!!(classA, listOf(classA), vMatchVararg)
        assertEquals(1, vMatchVararg.size)

        val vNotMatchSingle = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNameMatches("Class*").getShouldAssertion()!!(classA, listOf(classA), vNotMatchSingle)
        assertEquals(1, vNotMatchSingle.size)

        val vNotMatchList = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNameMatches(listOf("Class*")).getShouldAssertion()!!(classA, listOf(classA), vNotMatchList)
        assertEquals(1, vNotMatchList.size)

        val vNotMatchVararg = mutableListOf<String>()
        ClassesRuleBuilder(
            graph,
        ).should().notNameMatches("Class*", "Other*").getShouldAssertion()!!(
            classA,
            listOf(classA),
            vNotMatchVararg,
        )
