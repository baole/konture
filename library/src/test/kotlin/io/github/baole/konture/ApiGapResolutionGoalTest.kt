...
class ApiGapResolutionGoalTest {
    ...
    private fun testGraph(): ProjectGraph {
        val ann = AnnotationDeclaration("Entity", "javax.persistence.Entity", listOf(AnnotationArgumentDeclaration("table", "users")))
        val innerCls = ClassDeclaration(
            name = "InnerHelper",
        ...
        val topFunc = FunctionDeclaration(
            name = "parseUser",
        ...
        val topProp = PropertyDeclaration(
            name = "currentUser",
            type = "User",
            visibility = Visibility.PUBLIC,
            modifiers = emptySet(),
    ...
    fun `Fluent Scope extensions for file, function, and property work`() {
        ...
        val funcScope = KontureFunctionScope(graph.getAllModules().flatMap { m ->
            m.files.flatMap { f ->
        ...
        val propScope = KonturePropertyScope(graph.getAllModules().flatMap { m ->
            m.files.flatMap { f ->
