...
| **`failOnSeverity`** | `Severity.ERROR` | Configures the minimum severity threshold required to fail the build with an `AssertionError`.<br>• Supported values: `Severity.ERROR` (default, only ERROR violations fail the build), `Severity.WARNING` (ERROR and WARNING violations fail), `Severity.INFO` (all violations fail), or `null` (audit/dry-run mode, no violations fail the build). Can also be overridden via system property `-Dkonture.fail.on.severity=error|warning|info|none` or programmatically via `Konture.failOnSeverity`. |
